...
//------------------------------------------------------------------------------------------	
		public setupWithXMLString (__xmlString:string):void {
			...
			this.setupWithJSON (__jsonXML);													
		}
		
//------------------------------------------------------------------------------------------
		public setupWithJSON (__jsonXML:any):void {
            var __elementName:any;

            for (__elementName in __jsonXML) {
                this.m_tag = __elementName;
                this.m_text = "";

                var __value:any = __jsonXML[__elementName];

                console.log (": setupWithJSON: ", __elementName, __value, this.notObject (__value));
                
                this.m_attribsMap = new Map<string, any> ();

                if (this.notObject (__value)) {
                    this.m_text = "" + __value;

                    this.m_children = new Array<XSimpleXMLNode> ();
                } else {
                    if (Array.isArray (__value)) {
                        this.m_children = this.getArrayChildren (__elementName, __value);
                    } else {
                        this.m_text = __value["#text"] != undefined ? "" + __value["#text"] : "";

                        if (__value["@"] != undefined) {
                            var __attribs:any = __value["@"];
                        ...
                        this.m_children = this.getJSONChildren (__value);
                    }
                }
            }
        }
...
//------------------------------------------------------------------------------------------
        public getArrayChildren (__elementName:string, __jsonArray:Array<any>):Array<XSimpleXMLNode> {
            ...
            for (__element of __jsonArray) {
                console.log (": ", __elementName, __element, typeof __element);
                    
                if (this.notObject (__element)) {
                    var __value:any = {};
                    __value[__elementName] = __element;
                    __xmlNode.setupWithJSON (__value); 
                } else {
                    __xmlNode.setupWithJSON (__element);
                }
            ...
            for (__elementName in __jsonXML) {
                ...
                if (__elementName != "#text" && __elementName != "@") {
                    if (Array.isArray (__jsonXML[__elementName])) {
                        ...
                        var __xmlNode:XSimpleXMLNode = new XSimpleXMLNode ();
                        __xmlNode.setupWithJSON (__jsonXML[__elementName]);
                        __children.push (__xmlNode);
                    }
                }
            }
